...
int main( int argc, char * * argv ){
	...
	// open file
	f.open( "test_3.bmp", std::ios::in | std::ios::binary  );
	char * file = 0;
...
//	int image_height =		5;
	...
	if( bits_per_pixel == 24 || bits_per_pixel == 32 ){		
		for( int i = 0; i < (image_height * image_width * 3) - 2; i++ ){

			image_data[ i ] = file[ pixel_offset + i + 2 ];
			image_data[ i+1 ] = file[ pixel_offset + i + 1 ];
			image_data[ i+2 ] = file[ pixel_offset + i + 0 ];

			//std::cout << std::hex << (int)file[ pixel_offset + i + 2 ] << " " << (int)file[ pixel_offset + i + 1 ] << " " << (int)file[ pixel_offset + i ] << "\n";
